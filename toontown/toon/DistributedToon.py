...
def zonetp(area):
    """
    Sends the user to a playground or HQ based on acronym.
    """
    if area.lower() == 'help':
        return 'The current available inputs are ttc, dd, dg, mml, tb, ddl, sbhq, cbhq, lbhq, bbhq, oz, and gs.'
    locations = {
        'dd': 1000,
        'ttc': 2000,
        'tb': 3000,
    ...
    try:
        base.cr.playGame.getPlace().requestTeleport(ZoneUtil.getHoodId(locations[area.lower()]), locations[area.lower()], None, -1)
        return 'Teleporting to zone %s.' % locations[area]
    except KeyError:
            base.cr.playGame.getPlace().requestTeleport(ZoneUtil.getHoodId(int(area)), (int(area)), None, -1)
            return 'Teleporting to zone %s.' % area